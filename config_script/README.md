...
- 在项目开局时，可能会需要对多台设备进行统一配置的需求。这些设备拥有许多相同配置，比如用户信息、NTP、SNMP等配置。也有一些不同的配置，比如 IP 地址、主机名等配置。
...
3. 每天设备不相同的配置内容保存在 devices_vars.xlsx 文件中。每一行是一台设备的配置内容。每一列是相同变量的配置内容。
